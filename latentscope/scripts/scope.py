...
def scope(dataset_id, embedding_id, umap_id, cluster_id, cluster_labels_id, label, description, scope_id=None, sae_id=None):
    ...
    # Build the combined scope-input parquet directly from in-memory DataFrame
    # (no intermediate {scope_id}.parquet written to disk)
    ...
    input_parquet_path = os.path.join(directory, id + "-input.parquet")
    combined_df.to_parquet(input_parquet_path)

    scope["rows"] = len(scope_parquet)
    scope["columns"] = scope_parquet.columns.tolist()
    scope["size"] = os.path.getsize(input_parquet_path)
